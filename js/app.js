import {VoiceEngine} from './voice-engine.js?v=361';
import {Playback} from './playback.js?v=361';
import {VisualEngine} from './visual-engine.js?v=361';
import {COURSE,buildLesson,courseFocus} from './course-engine.js?v=361';
import {HEAD_COPY,buildHeadCopy} from './headcopy-engine.js?v=361';
import {buildCustom} from './session-builder.js?v=361';
import {exportWav,exportMp3,download} from './export-engine.js?v=361';
import {exportVideo} from './video-export.js?v=361';
...
function stopLoading(){
  setLoading(false,1,'','');
  const rt=$('#renderTelemetry');
  if(rt){rt.classList.add('hidden');rt.setAttribute('aria-hidden','true')}
}
...
function showVideoTelemetry(t){
  const box=$('#renderTelemetry');
  if(!box)return;
  box.classList.remove('hidden');
  box.setAttribute('aria-hidden','false');

  const labels=box.querySelectorAll('.render-metric small');
  if(labels.length>=4){
    labels[0].textContent='VIDEO';
    labels[1].textContent=lang()==='es'?'VELOCIDAD':'SPEED';
    labels[2].textContent=lang()==='es'?'RESTANTE':'REMAINING';
    labels[3].textContent='FRAMES';
  }

  $('#renderProcessed').textContent=`${fmt(t.processedSeconds)} / ${fmt(t.totalSeconds)}`;
  $('#renderFrames').textContent=`${Number(t.completedFrames||0).toLocaleString()} / ${Number(t.totalFrames||0).toLocaleString()}`;

  if(t.calibrated){
    $('#renderSpeed').textContent=`${Math.max(0,t.realtime||0).toFixed(1)}×`;
    $('#renderEta').textContent=fmtEta(t.etaSeconds);
  }else{
    $('#renderSpeed').textContent='—';
    $('#renderEta').textContent=lang()==='es'?'CALIBRANDO…':'CALIBRATING…';
  }

  const compat=t.mode==='compatibility';
  $('#loadingKicker').textContent=compat
    ? (lang()==='es'?'RENDER COMPATIBLE':'COMPATIBILITY RENDER')
    : (lang()==='es'?'RENDERIZANDO VIDEO':'RENDERING VIDEO');
}
...
async function loadLesson(n){
  ...
  try{
    ...
    console.error(err);
    $('#assetStatus').textContent=`CW Studio v3.6 · ${err.message||err}`;$('#assetStatus').classList.add('active');
...
async function loadBonus(id){
  ...
  try{
    ...
    console.error(err);stopLoading();$('#assetStatus').textContent=`CW Studio v3.6 · ${err.message||err}`;$('#assetStatus').classList.add('active');$('#assetStatus').classList.add('warning');
  }
}
...
async function togglePlay(){
  if(!timeline){$('#assetStatus').textContent='CW Studio v3.6 · no session loaded';return}
  ...
  try{
    ...
    resetPlayButton();console.error(err);$('#assetStatus').textContent=`CW Studio v3.6 · ${err.message||err}`;$('#assetStatus').classList.add('active');$('#assetStatus').classList.add('warning');
  }
}
  ...
  if(wasPlaying&&!wasPaused&&currentPosition<timeline.duration-.05){
    ...
    try{
      await playback.play(timeline,{
        ...
        tone:+$('#toneRange').value,
        startAt:currentPosition,
        onStatus:s=>console.debug('[CW Studio audio]',s),
        onTick:(t,a)=>{visual.setAnalyser(a);drawLoop(t)},
...
async function exportAudio(kind){
  ...
  try{
    const options={
      ...
      onStatus:s=>{$('#assetStatus').textContent=`CW Studio v3.6 · ${s}`},
...
$('#videoBtn').onclick=async()=>{
  ...
  const longVideo=timeline.duration>=15*60;
  try{
    if(longVideo){
      if(!window.showSaveFilePicker){
        throw new Error(lang()==='es'
          ?'Para videos de 15 minutos o más usá Edge/Chrome con guardado directo a disco.'
          :'For videos 15 minutes or longer, use Edge/Chrome with direct-to-disk saving.');
      }
      const handle=await window.showSaveFilePicker({
        suggestedName:`${exportBaseName()}.mp4`,
    ...
    const blob=await exportVideo(timeline,voice,visual,{
      ...
      fileWritable,
      onProgress:p=>setLoading(true,p,'',''),
      onTelemetry:t=>showVideoTelemetry(t),
      onFallback:err=>{
        const why=String(err?.message||err||'').slice(0,120);
        console.warn('Offline MP4 path failed:',why);
      },
      onStage:stage=>{
        console.debug('[CW Studio video]',stage);
        if(stage==='audio'){
          const box=$('#renderTelemetry');
          if(box){box.classList.add('hidden');box.setAttribute('aria-hidden','true')}
          $('#loadingKicker').textContent=lang()==='es'?'PREPARANDO VIDEO':'PREPARING VIDEO';
        }else if(stage==='encoder'){
          $('#loadingKicker').textContent=lang()==='es'?'INICIANDO ENCODER':'STARTING ENCODER';
        }else if(stage==='finalize'){
          $('#loadingKicker').textContent=lang()==='es'?'FINALIZANDO VIDEO':'FINALIZING VIDEO';
        }
      }
    });
    if(!blob?.saved){
