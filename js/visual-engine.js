...
export class VisualEngine{
  ...
  voiceLine(time,w,h){
    ...
    for(let i=0;i<N;i++){
      ...
      const amp=(20+92*e)*center;
      const y=cy+(raw*.72+harmonics*e*.22)*amp;
      out.push({x,y});
    }
    return out;
  }
  ...
  mnemonicShape(char,lang,w,h){
    ...
    switch(shape){
      ...
      case 'cat':
        return concat(P([[-.68,.34],[-.5,-.32],[-.25,-.58],[0,-.4],[.25,-.58],[.5,-.32],[.68,.34],[.22,.56],[-.26,.56],[-.68,.34]]),P([[.58,.2],[.92,-.05],[.8,-.36]]));
      case 'guitar':
        return concat(circle(cx-s*.22,cy+s*.2,s*.36,38),circle(cx+s*.14,cy-s*.12,s*.24,34),P([[.25,-.32],[.84,-.8],[.92,-.7],[.36,-.2]]));
      case 'axe':
      ...
      case 'giraffe':
        return P([[-.46,.72],[-.42,.05],[-.15,-.12],[-.06,-.74],[.22,-.74],[.18,-.26],[.58,-.1],[.43,.26],[.2,.22],[.18,.72],[-.46,.72]]);
      case 'jet':
      ...
      case 'cloud':
        return concat(circle(cx-s*.45,cy+s*.05,s*.28,34,2.8,6.2),circle(cx-s*.08,cy-s*.18,s*.34,40,3,6.2),circle(cx+s*.35,cy,s*.3,38,3.2,6.3),P([[.62,.28],[-.68,.28]]));
      case 'needle':
      ...
      case 'cheese':
        return concat(P([[-.72,.56],[-.5,-.32],[.32,-.64],[.78,.2],[.56,.62],[-.72,.56]]),circle(cx-s*.2,cy+s*.05,s*.08,18),circle(cx+s*.28,cy+s*.22,s*.07,18));
      case 'quill':
      ...
      case 'violin':
        return concat(circle(cx-s*.15,cy+s*.2,s*.25,34),circle(cx+s*.1,cy-s*.15,s*.19,34),P([[.18,-.28],[.74,-.82],[.84,-.72],[.3,-.18]]));
      case 'wok':
      ...
      case 'xylophone': {
        let p=[];for(let i=0;i<7;i++){const x=-.72+i*.24;p.push(...P([[x,-.56+i*.07],[x+.14,.56-i*.07]]))}
        return concat(p,P([[-.82,.74],[.78,-.72]]));
      }
      case 'shoe':
      ...
      case 'swan':
        return concat(circle(cx-s*.15,cy+s*.25,s*.38,55,.1,5.7),P([[.05,.15],[.18,-.46],[.46,-.74],[.68,-.56],[.5,-.4]]));
      case 'clover':
      ...
      case 'hand':
        return P([[-.58,.68],[-.58,-.05],[-.44,-.64],[-.26,-.64],[-.26,-.1],[-.16,-.8],[.02,-.8],[.02,-.08],[.14,-.72],[.3,-.7],[.28,-.02],[.42,-.54],[.58,-.48],[.52,.32],[.2,.72],[-.58,.68]]);
      case 'snail':
        return concat(circle(cx-s*.2,cy,s*.42,64),P([[-.6,.25],[.7,.25],[.84,.05],[.66,-.05],[.8,-.36],[.66,-.05]]));
      case 'scythe':
