import {scheduleText,scheduleCheckTone} from './morse-engine.js?v=18';
...
export async function exportVideo(tl,voice,visual,{lang='es',gender='female',tone=700,fps=30,onProgress=()=>{}}={}){
  ...
  for(const e of tl.events){if(e.type==='cw')scheduleText(ctx,master,e.data.text,base+e.start,{wpm:e.data.wpm||15,effectiveWpm:e.data.eff||15,tone:e.data.tone||tone,amp:.30});else if(e.type==='check')scheduleCheckTone(ctx,master,base+e.start,{amp:.14});else if(e.type==='voice'||e.type==='charVoice'){const id=e.type==='voice'?e.data.id:`char_${String(e.data.char).toLowerCase()}`;const b=await voice.buffer(ctx,id,lang,gender);if(b){const s=ctx.createBufferSource();s.buffer=b;s.connect(master);s.start(base+e.start)}}}
